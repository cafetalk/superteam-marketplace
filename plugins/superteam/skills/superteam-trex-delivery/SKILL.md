...
**release.md 四章结构**（#1 per-service / #2 submission / #3 iteration 统一）：`# 变更项`（单值「内容」列，按环境取值用 `{placeholder}`，多行内容 `<br>` 进单格；变更项表后若有占位符则渲一张 `| key | dev | beta | prod |`）· `# 数据契约`（## Redis Keys：Key 模式/用途/TTL，结构可扩展 DDL/Kafka）· `# 提测代码`（提测分支 + 主 MR + 修复 MR）· `# 关联任务`（迭代 + 任务列表，Linear 链接）。#3 另有 `# 发布执行`（manual 区）。

**两个平级文件**：`release.md` 由 skill 从 `changes.yaml` 渲染（唯一数据源）；`submission.md`（提测单：变更范围/关联 MR/自测记录/回滚预案/测试同学关注点）**由本 session 负责开发的 agent 手写**，skill 不生成、不覆盖。`changes.yaml` 每个 service：`changes[].value`（单值，可多行）+ 可选 `changes[].placeholders: [{key,dev,beta,prod}]` + 可选 `data_contract.redis_keys: [{key,purpose,ttl}]` + `mr` / `fix_mrs`。
...
- **提测 (v2)**：`uv run scripts/submit.py --changes <releases/{it}/submissions/{task}/changes.yaml> --releases-root <trex-releases clone> --repo-map <service>=<repo path> [--repo-map ...] [--no-mr] [--no-push] [--dry-run]`
  - 校验 changes.yaml（非法 abort）→ 渲染 #2 `submissions/{task}/release.md`（task 跨服务汇总）+ 每 service 渲染 #1 `<repo>/releases/{it}/{task}/release.md`（写进各微服务 dev_ 分支工作区，随代码提交、由提测 MR dev_→review_ 携带）。`--repo-map` 缺某 service → 只渲 #2 跳 #1（warn）。
...
- 汇总发布：`uv run scripts/release.py --batch <iteration> --beta beta_<date>_<keyword> --releases-root <...> [--dry-run] [--yes|--no-push|--no-mr]`（产出 `releases/<release-item>/RELEASE.md`；记录 MR `beta_<date>_<keyword> → master`，缺 `--beta` 退回 `dev_<batch>` fallback）
...
- **per-repo 明细**（`<repo>/releases/<release-item>/<date>_<name>.md`）写进微服务 repo（= `--repo`）工作区，由工程师随代码在 `dev_*`（提测 MR 源分支）一起提交；skill 只自动 push `trex-releases`。
- **trex-releases 记录镜像代码生命周期**（不直推 master）：
  - 提测：记录在本轮公共 `dev_<date>_<name>` 分支（与各微服务仓同名），MR `dev_<date>_<name> → review_<date>_<name>`。
  - 发布：`release --beta beta_<date>_<keyword>`，记录 MR `beta_<date>_<keyword> → master`（与代码发布同拍）。
  - `trex-releases/master` = 已发布记录的唯一真相；提测期记录不进 master。
