...
class TestCmdListMcp:
    ...
    def test_name_filter_passed(self):
        code, _lines = _run_mcp(["list", "--name", "张"], [FAKE_MEMBERS[0]])
        assert code == 0


class TestCmdListDirect:
    def test_no_filters(self):
        code, _lines, mock_q = _run_direct(["list"], FAKE_MEMBERS)
        assert code == 0
        mock_q.assert_called_once()

    def test_name_filter(self):
        code, _lines, mock_q = _run_direct(["list", "--name", "张"], [FAKE_MEMBERS[0]])
        assert code == 0
        assert "张" in str(mock_q.call_args)

    def test_role_filter(self):
        code, _lines, mock_q = _run_direct(["list", "--role", "engineer"], [FAKE_MEMBERS[0]])
        assert code == 0
        assert "engineer" in str(mock_q.call_args)

    def test_user_id_filter(self):
        code, _lines, mock_q = _run_direct(["list", "--user-id", "42"], [FAKE_MEMBERS[0]])
        assert code == 0
...
class TestCmdResolve:
    def test_resolve_mcp_mode(self):
        ...
        with patch.object(sys, "argv", ["list_members.py", "resolve", "张伟"]), \
             patch("list_members.env", return_value="postgresql://fake/db"), \
             patch("db._use_mcp", return_value=True), \
             patch("db.resolve_member", return_value={"user_id": 99, "match_type": "exact"}), \
             patch("builtins.print", side_effect=lambda *a, **kw: output_lines.append(a[0] if a else "")):
            code = lm.main()
        assert code == 0
