...
|---|---|---|
| **典型项目** | `trex-web` | `trex-core` / `trex-passport` |
...
| **对外暴露** | HTTP REST（含 OpenAPI 生成的 SDK） | Dubbo（生产者）；可选 GraphQL（trex-core 有 `core-graphql`） |
...
历史上有人质疑"为什么 trex-web 和 trex-core 模块切法完全不一样"。答案是：**两者职能不同，切法应该不同**：
