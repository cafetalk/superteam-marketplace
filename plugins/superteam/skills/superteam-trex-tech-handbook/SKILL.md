...
- [`frontend/08-test-handoff.md`](frontend/08-test-handoff.md) — 前端提测 / 发布分支（`dev_*` → `review_*` → `master`；与 common 发布路径差异见该章）
...
- [`frontend/11-quality-ops.md`](frontend/11-quality-ops.md) — 构建与发布（Vercel / OSS / Chrome Store）
- [`frontend/12-environments.md`](frontend/12-environments.md) — 多环境域名 / 分支 / 测试入口
- [`frontend/appendix/`](frontend/appendix/) — glossary, toolchain, [new-app-checklist](frontend/appendix/templates/new-app-checklist.md), [release-checklist](frontend/appendix/templates/release-checklist.md)
...
- **Frontend (M3)**: 12 章 + appendix 已填；含 ESLint 统一规范、错误展示 / GA 命名、覆盖率、CSP 基线、Extension 权限审计、CWV 与 Lighthouse CI 策略。
